...
def run_reorder(args, dtype, device):
    ...
    contig_kernel = tex.thd_reorder(rank_kernel, cu_seqlens, args.cp_size, True, total_tokens)
    ...
    return [
        (
            ...
            benchmark(
                lambda: tex.thd_reorder(x, cu_seqlens, args.cp_size, False, total_tokens),
                args.warmup,
                args.iters,
        ...
        (
            ...
            benchmark(
                lambda: tex.thd_reorder(rank_kernel, cu_seqlens, args.cp_size, True, total_tokens),
                args.warmup,
                args.iters,
...
def run_valid_copy(args, dtype, device):
    ...
    return [
        (
            "thd_valid_copy",
            benchmark(
                lambda: legacy_valid_copy(legacy_out, inp, cu_seqlens_padded, cu_seqlens),
                args.warmup,
                args.iters,
                "legacy_thd_valid_copy" if args.profile_nvtx else None,
            ),
            benchmark(
                lambda: tex.thd_valid_copy(kernel_out, inp, cu_seqlens_padded, cu_seqlens),
                args.warmup,
                args.iters,
...
def run_kernel_only(args, dtype, device):
    ...
    lse = torch.randn(args.batch, args.heads, args.seqlen, device=device, dtype=torch.float32)
    packed_lse = torch.randn(args.heads, total_tokens, device=device, dtype=torch.float32)
    ...
    return [
        ...
        (
            ...
            None,
            benchmark(
                lambda: tex.thd_read_second_half_lse(lse, cu_seqlens, False, args.seqlen // 2),
                args.warmup,
                args.iters,
