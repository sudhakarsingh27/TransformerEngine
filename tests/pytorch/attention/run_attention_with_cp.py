    ...
    if qkv_format == "bshd":
        ...
        if load_balancing_strategy is CPLoadBalancingStrategy.NO_LOAD_BALANCE:
            ...
            if "," in thd_seqlen_pattern:
                seqlens_q = torch.tensor(
                    [int(length) for length in thd_seqlen_pattern.split(",")],
                    dtype=torch.int32,
                )
...
def run_dpa_with_cp(
    ...
    if benchmark_iters > 0 and dtype == "fp8" and not (fp8_dpa or fp8_mha):
        raise ValueError("FP8 benchmarks require fp8_dpa=True or fp8_mha=True")
    ...
    # Preserve the historical benchmark window: input leaves are recreated for
    # each iteration, followed by one synchronized forward+backward measurement.
